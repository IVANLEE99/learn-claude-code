---
name: gen-img
description: Generate images using OpenAI-compatible Images API (gpt-image-2 / grok-imagine-image / grok-imagine-image-lite). Trigger when user says "generate image", "create image", "draw", "画图", "生成图片", "生图", "create a picture", or asks to generate/create/draw any visual content.
version: 1.1.0
---

# gen-img — Image Generation Skill

Generate images via OpenAI-compatible `/v1/images/generations` API and save locally.
...
|----------|----------|-------------|---------|
| `GEN_IMG_API_URL` | Yes | API base URL (no trailing slash) | `https://elysiver.h-e.top` |
| `GEN_IMG_API_KEY` | Yes | API key | `sk-...` |
| `GEN_IMG_MODEL` | No | Model name (default: `gpt-image-2`) | `gpt-image-2`, `grok-imagine-image`, `grok-imagine-image-lite` |

If not set in environment, read them from settings.json at runtime:
...
with open(os.path.expanduser('~/.claude/settings.json')) as f:
    s = json.load(f)
env = s.get('env', {})
print(env.get('GEN_IMG_API_URL', ''))
print(env.get('GEN_IMG_API_KEY', ''))
print(env.get('GEN_IMG_MODEL', 'gpt-image-2'))
"
```
...
- **quality** (optional): Image quality, default `auto`. Options: `auto`, `low`, `medium`, `high` (mainly for gpt-image models)
...
```bash
# Read API config (URL / KEY / MODEL from settings.json)
eval "$(python3 -c "
import json, os
with open(os.path.expanduser('~/.claude/settings.json')) as f:
    s = json.load(f)
env = s.get('env', {})
url = env.get('GEN_IMG_API_URL', '')
key = env.get('GEN_IMG_API_KEY', '')
model = env.get('GEN_IMG_MODEL', 'gpt-image-2')
print(f'API_URL=\"{url}\"')
print(f'API_KEY=\"{key}\"')
print(f'MODEL=\"{model}\"')
")"

curl -s --max-time 180 "${API_URL}/v1/images/generations" \
...
> **Note:** Some models (e.g. `grok-imagine-image`, `grok-imagine-image-lite`) may ignore or not require `quality` / `output_format`. Keep sending them for OpenAI-compatible payloads; the server will ignore unsupported fields.
...
|-------|------|---------|-------------|
| `model` | string | from `GEN_IMG_MODEL` | Model identifier |
...
- `grok-imagine-image-lite` — Grok Imagine Lite (cheaper)
...
- **model_not_found / 无可用渠道**: Model not enabled for this key/group — switch `GEN_IMG_MODEL` or API provider
...
- Model is always read from `GEN_IMG_MODEL` (settings.json / env), never hard-coded at call time
